...
wf::scene::surface_state_t &
wf::scene::surface_state_t::operator=(surface_state_t &&other) {
  if (current_buffer) {
...
wf::scene::wlr_surface_node_t::wlr_surface_node_t(wlr_surface *surface,
                                                  bool autocommit)
    : node_t(false), autocommit(autocommit) {
  this->surface = surface;
  this->ptr_interaction =
      std::make_unique<wlr_surface_pointer_interaction_t>(surface, this);
  this->tch_interaction = nullptr;

  this->on_surface_destroyed.set_callback([=](void *) {
    this->surface = NULL;
    this->ptr_interaction.reset();
    this->tch_interaction.reset();
...
wf::scene::wlr_surface_node_t::find_node_at(const wf::pointf_t &at) {
  if (!surface) {
    return {};
  }
...
wf::scene::wlr_surface_node_t::pointer_interaction() {
  return *this->ptr_interaction;
}

static wf::touch_interaction_t dummy_tch;
...
class wf::scene::wlr_surface_node_t::wlr_surface_render_instance_t
    : public render_instance_t {
  std::shared_ptr<wlr_surface_node_t> self;
  wf::signal::connection_t<wf::frame_done_signal> on_frame_done =
      [=](wf::frame_done_signal *ev) { self->send_frame_done(false); };
      ...
      [=](node_damage_signal *data) {
        if (self->surface) {
          ...
          const float output_scale =
              visible_on ? visible_on->handle->scale : 1.0;
          if (scale != output_scale) {
            data->region.expand_edges(
                std::ceil(std::abs(scale - output_scale)));
          }
        }
...
public:
  wlr_surface_render_instance_t(std::shared_ptr<wlr_surface_node_t> self,
    ...
    this->last_visibility |=
        wlr_box{INT_MIN / 2, INT_MIN / 2, INT_MAX, INT_MAX};
  }
  ...
  void render(const wf::scene::render_instruction_t &data) override {
    ...
    data.pass->add_texture(*self->to_texture(), data.target,
                           self->get_bounding_box(), data.damage);
  }

  void presentation_feedback(wf::output_t *output) override {
    if (self->surface) {
      wlr_presentation_surface_scanned_out_on_output(self->surface,
                                                     output->handle);
    }
  }
  ...
  void compute_visibility(wf::output_t *output,
                          wf::region_t &visible) override {
...
void wf::scene::wlr_surface_node_t::gen_render_instances(
  ...
  instances.push_back(std::make_unique<wlr_surface_render_instance_t>(
      std::dynamic_pointer_cast<wlr_surface_node_t>(this->shared_from_this()),
      damage, output));
}
