...
static std::string get_version_string() {
  return std::string(WAYFIRE_VERSION) + "-" + wf::version::git_commit + " (" +
         __DATE__ + ") branch " + wf::version::git_branch + " wlroots-" +
         WLR_VERSION_STR + " JFF";
}
...
static void print_help() {
  ...
  std::cout << " -c,  --config            specify config file to use "
            << "(overrides WAYFIRE_CONFIG_FILE from the environment)"
            << std::endl;
  ...
  std::cout << " -R,  --damage-rerender   rerender damaged regions"
            << std::endl;
  std::cout << " -l,  --legacy-wl-drm     use legacy drm for wayland clients"
            << std::endl;
...
static void wlr_log_handler(wlr_log_importance level, const char *fmt,
                            va_list args) {
...
// extern "C" {
// void __assert_fail(const char* expr, const char *filename, unsigned int line,
// const char *assert_func)
// {
// LOGE("Assertion failed: ", expr, " at ", filename, ":", line, " in ",
// assert_func); wf::print_trace(false); std::_Exit(-1);
// }
// }
...
int main(int argc, char *argv[]) {
  ...
  /* Don't crash on SIGPIPE, e.g., when doing IPC to a client whose fd has been
   * closed. */
  ...
  if (drm_fd < 0) {
    ...
    LOGW("Failed to open DRM render device, consider specifying "
         "WLR_RENDER_DRM_DEVICE."
         "Trying SW rendering instead.");
#else
    LOGE("Failed to open DRM render device, consider specifying "
         "WLR_RENDER_DRM_DEVICE."
         "If you want to use software rendering, ensure that wlroots has been "
         "compiled with udmabuf "
         "allocator support (available in wlroots >= 0.19.0) and recompile "
         "Wayfire.");
