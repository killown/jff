...
std::shared_ptr<wf::scene::output_node_t>
wf::output_impl_t::node_for_layer(wf::scene::layer layer) const {
...
void wf::output_impl_t::set_workspace_set(
    std::shared_ptr<workspace_set_t> wset) {
...
std::string wf::output_t::to_string() const { return handle->name; }
...
wf::geometry_t wf::output_t::get_layout_geometry() const {
  wlr_box box;
  wlr_output_layout_get_box(wf::get_core().output_layout->get_handle(), handle,
                            &box);
...
bool wf::output_impl_t::can_activate_plugin(uint32_t caps, uint32_t flags) {
...
bool wf::output_impl_t::can_activate_plugin(wf::plugin_activation_data_t *owner,
                                            uint32_t flags) {
  if (!owner) {
    return false;
  }
...
bool wf::output_impl_t::activate_plugin(wf::plugin_activation_data_t *owner,
                                        uint32_t flags) {
...
bool wf::output_t::is_inhibited() const { return this->inhibited; }

namespace wf {
void output_impl_t::add_key(option_sptr_t<keybinding_t> key,
...
void output_impl_t::add_activator(option_sptr_t<activatorbinding_t> activator,
...
void collect_output_nodes_recursive(
  ...
  if (auto output_node =
          std::dynamic_pointer_cast<scene::output_node_t>(root)) {
...
std::vector<std::shared_ptr<scene::output_node_t>>
collect_output_nodes(wf::scene::node_ptr root, wf::output_t *output) {
