...
struct wlr_idle_inhibitor_t : public wf::idle_inhibitor_t {
  ...
  wlr_idle_inhibitor_t(wlr_idle_inhibitor_v1 *wlri) {
    on_destroy.set_callback([&](void *) { delete this; });
...
void wf::compositor_core_impl_t::init() {
  ...
  wf::option_wrapper_t<bool> disable_primary_selection{
      "workarounds/disable_primary_selection"};
  ...
  protocols.output_manager =
      wlr_xdg_output_manager_v1_create(display, output_layout->get_handle());
  ...
  wf::option_wrapper_t<bool> enable_input_method_v2{
  ...
  protocols.foreign_v1 =
      wlr_xdg_foreign_v1_create(display, protocols.foreign_registry);
  protocols.foreign_v2 =
      wlr_xdg_foreign_v2_create(display, protocols.foreign_registry);
...
void wf::compositor_core_impl_t::increase_nofile_limit() {
  if (getrlimit(RLIMIT_NOFILE, &user_maxfiles) != 0) {
    LOGE("Failed to getrlimit(RLIMIT_NOFILE), not increasing maximum open file "
         "descriptors. Might cause"
    ...
    if (setrlimit(RLIMIT_NOFILE, &max_files) != 0) {
      LOGE("Failed to setrlimit(RLIMIT_NOFILE), not increasing maximum open "
           "file descriptors. Might "
...
void wf::compositor_core_impl_t::restore_nofile_limit() {
  if (setrlimit(RLIMIT_NOFILE, &user_maxfiles) != 0) {
    LOGE("Failed to setrlimit(RLIMIT_NOFILE), could not restore maximum open "
         "file descriptors.");
  }
}
...
void wf::compositor_core_impl_t::shutdown() {
  if (this->state < compositor_state_t::RUNNING) {
    // During initialization, shut down is a bit more complicated. We can
    // deallocate core, but since we have not started the event loop, we can
    // exit immediately.
    deallocate_core();
    std::exit(0);
  }

  // We might get multiple signals in some scenarios. Shutdown only on the first
  // instance.
...
void wf::compositor_core_impl_t::fini() {
  ...
  // Shut down xwayland first, otherwise, wlroots will attempt to restart it
  // when we kill it via wl_display_destroy_clients().
  wf::fini_xwayland();
...
wlr_seat *wf::compositor_core_impl_t::get_current_seat() { return seat->seat; }
...
pid_t wf::compositor_core_impl_t::run(std::string command) {
  ...
  if (ret == -1) {
    LOGE("wf::compositor_core_impl_t::run: failed to create pipe2: ",
         strerror(errno));
    return 0;
  }
  ...
  if (!pid) {
    ...
    // Return 0 if the child process didn't run or didn't exit normally, or
    // returns a non-zero return value.
    pid_t child_pid{};
    if (WIFEXITED(status) && (WEXITSTATUS(status) == 0)) {
      int ret = read(pipe_fd[READ_END], &child_pid, sizeof(child_pid));
      if (ret != sizeof(child_pid)) {
        // This is consider to be an error (even though theoretically a partial
        // read would require an attempt to continue).
        child_pid = 0;
        if (ret == -1) {
          ...
          LOGE("wf::compositor_core_impl_t::run(\"", command,
               "\"): short read of PID from pipe end, got ",
               std::to_string(ret), " bytes");
        }
      }
    }
...
void wf::start_move_view_to_wset(
    wayfire_toplevel_view v, std::shared_ptr<wf::workspace_set_t> new_wset) {
...
void wf::move_view_to_output(wayfire_toplevel_view v, wf::output_t *new_output,
                             uint32_t flags) {
  ...
  wf::dassert(!v->parent,
              "Cannot move a dialog to a different output than its parent!");

  auto old_output = v->get_output();
  auto old_wset = v->get_wset();
  auto new_wset = new_output->wset();
  const bool reconfigure =
      (flags & VIEW_TO_OUTPUT_FLAG_RECONFIGURE) && (old_output != nullptr);
  const bool same_workspace = (flags & VIEW_TO_OUTPUT_FLAG_SAME_WORKSPACE) &&
                              reconfigure && (old_wset != nullptr);
  ...
  if (reconfigure) {
    ...
    if (same_workspace) {
      ...
      new_ws = {std::min(old_ws.x, new_grid_size.width),
...
const std::shared_ptr<wf::scene::root_node_t> &
wf::compositor_core_impl_t::scene() {
  return scene_root;
}
...
wf::wayland_global_filter_t::~wayland_global_filter_t() { unset_filter(); }

bool wf::wayland_global_filter_t::check_global(const wl_client *client,
...
void wf::compositor_core_impl_t::register_filter(
    wayland_global_filter_t *filter) {
  if (std::find(wayland_global_filters.begin(), wayland_global_filters.end(),
                filter) == wayland_global_filters.end()) {
...
void wf::compositor_core_impl_t::unregister_filter(
    wayland_global_filter_t *filter) {
  auto it = std::find(wayland_global_filters.begin(),
                      wayland_global_filters.end(), filter);
...
bool wf::compositor_core_impl_t::global_filter(const wl_client *client,
                                               const wl_global *global,
                                               void *data) {
  auto *self = static_cast<wf::compositor_core_impl_t *>(data);
  return std::all_of(self->wayland_global_filters.begin(),
                     self->wayland_global_filters.end(),
                     [&](wayland_global_filter_t *filter) {
                       return filter->check_global(client, global);
                     });
}

std::unique_ptr<wf::wayland_global_filter_t>
wf::compositor_core_t::create_global_filter() {
  return std::unique_ptr<wf::wayland_global_filter_t>(
      new wf::wayland_global_filter_t());
}
...
wf::compositor_core_t &wf::get_core() { return wf::compositor_core_t::get(); }
...
std::unique_ptr<wf::compositor_core_impl_t>
    wf::compositor_core_impl_t::static_core;

// TODO: move this to a better location
wf_runtime_config runtime_config;

std::shared_ptr<wf::config::option_base_t>
wf::detail::load_raw_option(const std::string &name) {
