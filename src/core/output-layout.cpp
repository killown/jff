...
public:
  wlr_output_state_setter_t() { wlr_output_state_init(&pending); }

  ~wlr_output_state_setter_t() { wlr_output_state_finish(&pending); }
  ...
  operator=(const wlr_output_state_setter_t &) = delete;
...
std::string
wf::layout_detail::wl_transform_to_string(wl_output_transform transform) {
...
std::string_view
wf::layout_detail::get_output_source_name(output_image_source_t source) {
  switch (source) {
  ...
  wl_list_for_each(mode, &output->modes, link) {
    if ((mode->width == reference.width) &&
      ...
      if ((reference.refresh == 0) && mode->preferred) {
        // If there is a preferred mode and there is no refresh configured, pick
        // preferred mode.
        return mode;
      }
      ...
      const int best_so_far =
          best ? std::abs(best->refresh - target_refresh) : INT_MAX;
...
static bool parse_modeline(const char *modeline, drmModeModeInfo &mode) {
  ...
  snprintf(mode.name, sizeof(mode.name), "%dx%d@%d", mode.hdisplay,
...
void transfer_views(wf::output_t *from, wf::output_t *to) {
  ...
  if (to) {
    ...
    for (auto &view : views) {
      unsigned flags =
          VIEW_TO_OUTPUT_FLAG_RECONFIGURE | VIEW_TO_OUTPUT_FLAG_SAME_WORKSPACE;
      move_view_to_output(view, to, flags);
    }
  }

  // Step 2: Ensure none of the remaining views have an invalid output.
  // Note that all views in workspace sets will have their output reassigned
  // automatically by the workspace-set impl.
  std::vector<std::shared_ptr<wf::view_interface_t>> non_ws_views;
  for (auto &view : wf::get_core().get_all_views()) {
    if ((view->get_output() == from) &&
        (!toplevel_cast(view) || !toplevel_cast(view)->get_wset())) {
      // Take a ref, so that the view doesn't get destroyed while we're doing
  ...
  for (auto &view : non_ws_views) {
    if (view->role == VIEW_ROLE_DESKTOP_ENVIRONMENT) {
      // typically: layer-shell views that should be closed, since they are tied
      // to a single output
      view->close();
...
struct output_layout_output_t {
  ...
  output_layout_output_t(wlr_output *handle) {
    ...
    formats_for_depth[10] = {DRM_FORMAT_XRGB2101010, DRM_FORMAT_XBGR2101010,
                             DRM_FORMAT_XRGB8888};
  }

  ~output_layout_output_t() {
    on_destroy.disconnect();
    on_request_state.disconnect();
  ...
  void handle_mode_changed() {
    ...
    if (config.count(handle) &&
      ...
      if (output && (output->get_screen_size() != get_effective_size())) {
        ...
        // Emit the output-layout-configuration-changed signal as well, which is
        // usually emitted for all changed outputs together in
        // output-layout::apply_state(). However, resizing nested backends does
        // not use apply_state(), hence we have to emit the signal manually.
  ...
  wlr_output_mode select_default_mode() {
    ...
    if (!wl_list_empty(&handle->modes)) {
      mode = wl_container_of(handle->modes.prev, mode, link);
      return *mode;
    }

    wlr_output_mode default_mode;
    default_mode.width = handle->width > 0 ? handle->width : 1200;
    default_mode.height = handle->height > 0 ? handle->height : 720;
    default_mode.refresh = handle->refresh > 0 ? handle->refresh : 0;
  ...
  void ensure_wayfire_output(const wf::dimensions_t &effective_size) {
    ...
    this->output = std::make_unique<wf::output_impl_t>(handle, effective_size);
    auto wo = output.get();
    ...
    wlr_output *focused = get_core().seat->get_active_output()
                              ? get_core().seat->get_active_output()->handle
                              : nullptr;
  ...
  void destroy_wayfire_output() {
    ...
    transfer_views(wo,
                   shutdown ? nullptr : get_core().seat->get_active_output());
  ...
  void refresh_custom_modes() {
    auto section = wf::get_core().config_backend->get_output_section(handle);
  ...
  bool test_state(const output_state_t &state) { return true; }
  ...
  void apply_mode(const wlr_output_mode &mode, bool custom_mode) {
    if (handle->current_mode) {
      ...
      if ((handle->current_mode->width == mode.width) &&
          ...
          ((handle->adaptive_sync_status == WLR_OUTPUT_ADAPTIVE_SYNC_ENABLED) ==
           current_state.vrr) &&
    ...
    if (built_in) {
      ...
      LOGI("Couldn't find matching mode ", mode.width, "x", mode.height, "@",
           mode.refresh / 1000.0, " for output ", handle->name,
           ". Trying to use custom mode", "(might not work)");

      wlr_output_state_set_custom_mode(&pending_state.pending, mode.width,
                                       mode.height, mode.refresh);
    }

    pending_state.commit(handle);

    const bool adaptive_sync_enabled =
        (handle->adaptive_sync_status == WLR_OUTPUT_ADAPTIVE_SYNC_ENABLED);

    if (adaptive_sync_enabled != current_state.vrr) {
      wlr_output_state_set_adaptive_sync_enabled(&pending_state.pending,
                                                 current_state.vrr);
      if (pending_state.test_and_commit(handle)) {
        LOGD("Changed adaptive sync on output: ", handle->name, " to ",
             current_state.vrr);
      } else {
    ...
    if (current_state.depth != current_bit_depth) {
      for (auto fmt : formats_for_depth[current_state.depth]) {
        ...
        if (pending_state.test_and_commit(handle)) {
          current_bit_depth = current_state.depth;
          LOGD("Set output format to ", get_format_name(fmt), " on output ",
               handle->name);
          break;
        }

        LOGD("Failed to set output format ", get_format_name(fmt),
             " on output ", handle->name);
      }
    }
  }
  ...
  void render_output(wlr_texture *texture) {
    // TODO: use render-manager's functions, apply gamma, use our normal pass
    // functions.
    struct wlr_render_pass *pass =
        wlr_output_begin_render_pass(handle, &pending_state.pending, NULL);
    if (pass == NULL) {
      return;
    }
  ...
  void handle_frame() {
    auto wo = get_core().output_layout->find_output(current_state.mirror_from);
    if (!wo) {
  ...
  void setup_mirror() {
    ...
    if (wo) {
      auto config = get_core().output_layout->get_current_configuration();
  ...
  wf::dimensions_t get_effective_size() {
    ...
    wlr_output_effective_resolution(handle, &effective_size.width,
...
public:
  impl(wlr_backend *backend) {
    on_new_output.set_callback(
        [=](void *data) { add_output((wlr_output *)data); });
  ...
  void apply_wlr_configuration(wlr_output_configuration_v1 *wlr_configuration,
                               bool test_only) {
    auto configuration =
  ...
  void ensure_noop_output() {
    if (!noop_output ||
        (noop_output->current_state.source != OUTPUT_IMAGE_SOURCE_SELF)) {
    ...
    if (!noop_output) {
      ...
      if (!wlr_output_init_render(handle, get_core().allocator,
                                  get_core().renderer)) {
  ...
  void add_output(wlr_output *output) {
    LOGI("new output: ", output->name, " (\"", output->make, " ", output->model,
         " ", output->serial, "\")");
    ...
    if (output->non_desktop) {
      ...
      if (get_core().protocols.drm_v1) {
        ...
        wlr_drm_lease_v1_manager_offer_output(get_core().protocols.drm_v1,
                                              output);
      }
    ...
    if (!wlr_output_init_render(output, get_core().allocator,
                                get_core().renderer)) {
    ...
    lo->on_destroy.set_callback(
        [output, this](void *) { remove_output(output); });
    ...
    lo->on_request_state.set_callback([=](void *data) {
      ...
      for (auto &output : outputs) {
        output.second->emit_configuration_changed(wf::OUTPUT_POSITION_CHANGE |
                                                  wf::OUTPUT_MODE_CHANGE);
      }
   ...
   */
  wf::geometry_t
  calculate_geometry_from_state(const output_state_t &state) const {
  ...
  calculate_fixed_geometries(const output_configuration_t &config) {
  ...
  void apply_configuration(const output_configuration_t &config) {
    ...
    for (auto &entry : config) {
      ...
      if (entry.second.position.is_automatic_position()) {
        ...
        LOGC(OUTPUT, "\t  position: ", entry.second.position.get_x(), ", ",
             entry.second.position.get_y());
      }
      ...
      if (entry.second.source == OUTPUT_IMAGE_SOURCE_NONE) {
        ...
        LOGC(OUTPUT, "\t  mode: ", entry.second.mode.width, "x",
             entry.second.mode.height, "@", entry.second.mode.refresh,
             entry.second.uses_custom_mode ? " (custom)" : "");
      }

      LOGC(OUTPUT, "\t  scale: ", entry.second.scale);
      LOGC(OUTPUT, "\t  transform: ",
           layout_detail::wl_transform_to_string(entry.second.transform));
    ...
    for (auto &entry : config) {
      ...
      if (state.source & OUTPUT_IMAGE_SOURCE_SELF &&
          !entry.second.position.is_automatic_position()) {
        ++count_enabled;
        wlr_output_layout_add(output_layout, handle, state.position.get_x(),
                              state.position.get_y());
        lo->apply_state(state);
      }
    }
    ...
    /* Fifth: emit configuration-changed again for dynamically-positioned
     * outputs, because their position might have changed. */
    ...
    if (count_enabled > 0) {
      ...
      timer_remove_noop.set_timeout(1000, [=]() { remove_noop_output(); });
    }

    idle_update_configuration.run_once([=]() { send_wlr_configuration(); });
  }

  void emit_configuration_changed_for_dynamic_outputs(
  ...
  void send_wlr_configuration() {
    ...
    for (auto &output : outputs) {
      auto head = wlr_output_configuration_head_v1_create(wlr_configuration,
                                                          output.first);
    ...
    wlr_output_manager_v1_set_configuration(output_manager, wlr_configuration);
  }
  ...
  wf::output_t *get_output_coords_at(const wf::pointf_t &origin,
                                     wf::pointf_t &closest) {
    wlr_output_layout_closest_point(output_layout, NULL, origin.x, origin.y,
                                    &closest.x, &closest.y);
