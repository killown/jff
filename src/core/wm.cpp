...
static void idle_shutdown(void *data) { wf::get_core().shutdown(); }

void wayfire_exit::init() {
  wf::option_wrapper_t<wf::keybinding_t> wayifre_exit_binding{"core/exit"};

  key = [](const wf::keybinding_t &) {
    auto output_impl = static_cast<wf::output_impl_t *>(
        wf::get_core().seat->get_active_output());
...
void wayfire_exit::fini() { output->rem_binding(&key); }
...
void wayfire_close::fini() { output->rem_binding(&callback); }
...
void wayfire_focus::init() {
  on_pointer_button =
      [=](wf::input_event_signal<wlr_pointer_button_event> *ev) {
        ...
        if ((!focus_modifiers &&
            ...
            !tmp->get_value().has_match(
                wf::buttonbinding_t(0, ev->event->button))) {
          return;
        }
  ...
  // Touch handlers removed to wipe touch support
}

void wayfire_focus::fini() { on_pointer_button.disconnect(); }

bool wayfire_focus::check_focus_surface(wayfire_view view) {
  auto &core = wf::get_core();
  if (!view || !view->is_mapped() || !core.seat->get_active_output() ||
      !core.seat->get_active_output()->can_activate_plugin(
