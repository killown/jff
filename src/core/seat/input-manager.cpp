...
void wf::input_manager_t::handle_new_input(wlr_input_device *dev) {
  ...
  configure_input_device(input_devices.back());
}

void wf::input_manager_t::configure_input_device(
    std::unique_ptr<wf::input_device_impl_t> &device) {
  auto section = wf::get_core().config_backend->get_input_device_section(
      "input-device", device->get_wlr_handle());
  device->reconfigure_device(section);
}
...
void wf::input_manager_t::handle_input_destroyed(wlr_input_device *dev) {
  ...
  auto it =
      std::remove_if(input_devices.begin(), input_devices.end(),
