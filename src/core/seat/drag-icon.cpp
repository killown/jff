...
class dnd_root_icon_root_node_t : public floating_inner_node_t {
  class dnd_icon_root_render_instance_t : public render_instance_t {
    ...
    wf::signal::connection_t<node_damage_signal> on_damage =
        [=](node_damage_signal *data) { push_damage(data->region); };

    std::weak_ptr<dnd_root_icon_root_node_t> _self;

  public:
    dnd_icon_root_render_instance_t(dnd_root_icon_root_node_t *self,
                                    damage_callback push_damage) {
      this->_self = std::dynamic_pointer_cast<dnd_root_icon_root_node_t>(
...
public:
  ...
  void gen_render_instances(std::vector<render_instance_uptr> &instances,
    ...
    instances.push_back(
        std::make_unique<dnd_icon_root_render_instance_t>(this, push_damage));
  }
...
wf::drag_icon_t::drag_icon_t(wlr_drag_icon *ic) : icon(ic) {
  ...
  auto main_node =
      std::make_shared<scene::wlr_surface_node_t>(icon->surface, true);
...
void wf::drag_icon_t::update_position() {
  ...
  dmg_region |= last_box;
  last_box = wf::construct_box(get_position(), {icon->surface->current.width,
                                                icon->surface->current.height});
  dmg_region |= last_box;
