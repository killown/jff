...
void wf::seat_t::set_active_node(wf::scene::node_ptr node,
  ...
  priv->set_keyboard_focus(
      focus.node ? focus.node->shared_from_this() : nullptr, reason);
}
...
wf::output_t *wf::seat_t::get_active_output() { return priv->active_output; }
...
void wf::seat_t::impl::check_update_active_view(wf::scene::node_ptr new_focus) {
  static wf::option_wrapper_t<bool> keep_last_focus_activated{
      "workarounds/keep_last_toplevel_activated"};
  ...
  const bool do_change_activated =
      ((view == nullptr) || toplevel_cast(view)) || !keep_last_focus_activated;
...
static wayfire_view pick_topmost_focusable(wayfire_view view) {
  ...
  auto it =
      std::find_if(all_views.begin(), all_views.end(), [](wayfire_view v) {
        return v->get_keyboard_focus_surface() != NULL;
      });
...
void wf::seat_t::focus_view(wayfire_view v) {
  static wf::option_wrapper_t<bool> all_dialogs_modal{
...
void wf::seat_t::refocus() {
  ...
  auto focus =
      wf::get_core().scene()->keyboard_refocus(priv->active_output).node;
  LOGC(KBD, "Output ", priv->active_output->to_string(),
       " refocusing: choosing node ", focus);
...
uint32_t wf::seat_t::get_keyboard_modifiers() { return priv->get_modifiers(); }
...
void wf::seat_t::notify_activity() {
  wlr_idle_notifier_v1_notify_activity(wf::get_core().protocols.idle_notifier,
                                       this->seat);
...
std::vector<uint32_t> wf::seat_t::get_pressed_keys() {
  std::vector<uint32_t> pressed_keys{priv->pressed_keys.begin(),
                                     priv->pressed_keys.end()};
  return pressed_keys;
}

wf::seat_t::seat_t(wl_display *display, std::string name)
    : seat(wlr_seat_create(display, name.c_str())) {
  priv = std::make_unique<impl>();
  priv->seat = seat;
  priv->cursor = std::make_unique<wf::cursor_t>(this);
  priv->lpointer = std::make_unique<wf::pointer_t>(wf::get_core_impl().input,
  ...
  priv->request_set_selection.set_callback([&](void *data) {
    ...
    wlr_seat_set_selection(wf::get_core().get_current_seat(), ev->source,
                           ev->serial);
  });
  ...
  priv->on_wlr_keyboard_grab_end.set_callback([&](void *) {
    if (priv->keyboard_focus &&
        dynamic_cast<wlr_view_keyboard_interaction_t *>(
            &priv->keyboard_focus->keyboard_interaction())) {
  ...
  priv->on_wlr_pointer_grab_end.set_callback([&](void *) {
    ...
    if (auto focus = priv->lpointer->get_focus()) {
      if (dynamic_cast<wlr_surface_pointer_interaction_t *>(
              &focus->pointer_interaction())) {
        wf::pointf_t local = get_node_local_coords(
            focus.get(), priv->cursor->get_cursor_position());
  ...
  priv->on_new_device = [&](wf::input_device_added_signal *ev) {
    ...
    case WLR_INPUT_DEVICE_KEYBOARD:
      this->priv->keyboards.emplace_back(
          std::make_unique<wf::keyboard_t>(ev->device->get_wlr_handle()));
  ...
  priv->on_root_node_updated.set_callback(
      [=](scene::root_node_update_signal *ev) {
...
void wf::seat_t::impl::validate_drag_request(
    wlr_seat_request_start_drag_event *ev) {
...
void wf::seat_t::impl::set_keyboard(wf::keyboard_t *keyboard) {
  ...
  wlr_seat_set_keyboard(
      seat, keyboard ? wlr_keyboard_from_input_device(keyboard->device) : NULL);
}
...
void wf::seat_t::impl::force_release_keys() {
  if (this->keyboard_focus) {
    for (auto key : this->pressed_keys) {
      ...
      this->keyboard_focus->keyboard_interaction().handle_keyboard_key(
          wf::get_core().seat.get(), ev);
    }
  }
}
...
namespace wf {
wlr_input_device *input_device_t::get_wlr_handle() { return handle; }
...
bool input_device_t::set_enabled(bool enabled) {
  ...
  libinput_device_config_send_events_set_mode(
      dev, enabled ? LIBINPUT_CONFIG_SEND_EVENTS_ENABLED
...
wf::input_device_impl_t::~input_device_impl_t() { this->handle->data = NULL; }

void wf::input_device_impl_t::map_to_output(
  ...
  if (mapped_output.empty()) {
    if (handle->type == WLR_INPUT_DEVICE_POINTER) {
      mapped_output =
          nonull(wlr_pointer_from_input_device(handle)->output_name);
    }
  }

  auto wo = wf::get_core().output_layout->find_output(mapped_output);
  if (wo) {
    LOGC(INPUT_DEVICES, "Mapping input ", handle->name, " to output ",
         wo->to_string(), ".");
...
static wf::pointf_t to_local_recursive(wf::scene::node_t *node,
                                       wf::pointf_t point) {
  if (node->parent()) {
