...
/**
 * The frame-done signal is emitted on an output when the frame has been
 * completed (regardless of whether new content was painted or not).
 */
...
public:
  ...
  /**
   * @return The current render pass, NULL if no rendering operations are
   * currently active on the output.
   */
  ...
  /**
   * @return The current wlr_color_transform from the icc_profile option, or
   * NULL if none is set.
   */
  ...
  /**
   * Inform Wayfire whether a depth buffer is required for rendering on the
   * default framebuffer for each output.
   */
