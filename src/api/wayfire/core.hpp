...
enum class compositor_state_t {
  ...
  START_BACKEND,
  /**
   * Both core and wlroots have finished initialization. Now plugins are being
   * started.
   */
  START_PLUGINS,
  /**
 ...
 *
 * This can be used to hide certain globals from clients, for example to
 * implement the security-context-v1 protocol, or make sure that only certain
 * clients have access to privileged protocols.
 */
class wayland_global_filter_t final {
public:
  using filter_callback =
      std::function<bool(const wl_client *client, const wl_global *global)>;

  /**
   * Set a new global filter. By default, all globals are advertised to all
   * clients.
   *
   * In case multiple plugins create filters, a global is advertised to a client
...
public:
   ...
   * Note that when transferring a grab, synthetic button release/etc. events
   * are sent to the old pointer and touch focus nodes (except if they are not
   * RAW_INPUT nodes).
   *
   * The grab node, if it is not RAW_INPUT, will also not receive button release
   * events for buttons pressed before it grabbed the input, if it does not have
   * the RAW_INPUT flag.
   *
  ...
  /**
   * Checks whether the current renderer is a Pixman renderer.
   */
  bool is_pixman() const;
...
enum {
  /**
   * Adjust the view geometry for the new output and clamp it to the output
   * geometry so it is at an expected size and position.
   */
  VIEW_TO_OUTPUT_FLAG_RECONFIGURE = 1 << 0,
  /**
   * When reconfiguring, also move the view to the same workspace as it is
   * currently on. Only meaningful if VIEW_TO_OUTPUT_FLAG_RECONFIGURE is also
   * set.
   */
 ...
 */
void move_view_to_output(wayfire_toplevel_view v, wf::output_t *new_output,
                         uint32_t flags);

/**
 * Change the view's output to new_output. Equivalent to calling the
 * move_view_to_output with the RECONFIGURE flag set to the specified value and
 * no other flags set.
 */
inline void move_view_to_output(wayfire_toplevel_view v,
...
/**
 * Start a move of a view to a new workspace set (and thus potentially to a new
 * output). Note that the view will be removed from its current workspace set
 * and added to the new one. However, the view's geometry will not be adjusted,
 * and neither will the view_moved_to_wset_signal be emitted (in contrast to
 * move_view_to_output, which emits view_moved_to_wset_signal).
 */
void start_move_view_to_wset(wayfire_toplevel_view v,
