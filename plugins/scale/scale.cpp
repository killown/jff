...
struct view_scale_data {
  ...
  bool was_minimized =
      false; /* flag to indicate if this view was originally minimized */
};
...
public:
  ...
  void init() override {
    ...
    grab = std::make_unique<wf::input_grab_t>(SCALE_TRANSFORMER, output, this,
                                              this);
  ...
  void setup_workspace_switching() {
    ...
    workspace_bindings->setup(
        [&](wf::point_t delta, wayfire_toplevel_view view, bool only_view) {
  ...
  wf::signal::connection_t<scale_update_signal> update_cb =
  ...
  void handle_pointer_motion(wf::pointf_t to_f, uint32_t time) override {
    ...
    if (!drag_helper->view && last_selected_view &&
        drag_helper->should_start_pending_drag(to)) {
      wf::move_drag::drag_options_t opts;
      opts.join_views = true;
      opts.enable_snap_off = true;
      opts.snap_off_threshold = 200;

      // We want to receive raw inputs (e.g. no fake pointer releases) in case
      // the view is moved to another output.
  ...
  wf::point_t get_view_main_workspace(wayfire_toplevel_view view) {
    ...
    return wf::point_t{ws.x + (int)std::floor((double)center.x / og.width),
  ...
  wayfire_toplevel_view find_view_in_grid(int row, int col) {
    for (auto &view : scale_data) {
      if ((view.first->parent == nullptr) &&
          ...
          ((view.second.row == row) && (view.second.col == col))) {
        return view.first;
      }
    }
  ...
  void handle_keyboard_key(wf::seat_t *, wlr_keyboard_key_event ev) override {
    ...
    if (!current_row_sizes.empty()) {
      next_row =
          (next_row + current_row_sizes.size()) % current_row_sizes.size();
  ...
  std::vector<wayfire_toplevel_view> get_all_workspace_views() {
    return output->wset()->get_views(
        (include_minimized ? 0 : wf::WSET_EXCLUDE_MINIMIZED) |
        wf::WSET_MAPPED_ONLY);
  }
  ...
  bool should_scale_view(wayfire_toplevel_view view) {
    ...
    return std::find(views.begin(), views.end(),
                     wf::find_topmost_parent(view)) != views.end();
  }

  /* Convenience assignment function */
  void setup_view_transform(view_scale_data &view_data, double scale_x,
                            double scale_y, double translation_x,
                            double translation_y, double target_alpha) {
  ...
  static bool view_compare_x(const wayfire_toplevel_view &a,
                             const wayfire_toplevel_view &b) {
  ...
  static bool view_compare_y(const wayfire_toplevel_view &a,
                             const wayfire_toplevel_view &b) {
    auto vg_a = a->get_geometry();
  ...
  std::vector<std::vector<wayfire_toplevel_view>>
  view_sort(std::vector<wayfire_toplevel_view> &views) {
  ...
  void filter_views(std::vector<wayfire_toplevel_view> &views) {
    ...
    for (auto view : filtered_views) {
      for (auto v : view->enumerate_views(true)) {
        ...
        if (view_data.visibility ==
            view_scale_data::view_visibility_t::VISIBLE) {
          view_data.visibility = view_scale_data::view_visibility_t::HIDING;
    ...
    if (!current_focus_view) {
      std::sort(views.begin(), views.end(),
                [=](wayfire_toplevel_view a, wayfire_toplevel_view b) {
                  ...
                  return wf::get_focus_timestamp(a) >
                         wf::get_focus_timestamp(b);
                });
  ...
  void layout_slots(std::vector<wayfire_toplevel_view> views) {
    ...
    const double scaled_height = std::max(
        (double)(workarea.height - (cnt_rows + 1) * spacing) / cnt_rows, 1.0);
    current_row_sizes.clear();

    for (size_t i = 0; i < cnt_rows; i++) {
      size_t cnt_cols = sorted_rows[i].size();
      current_row_sizes.push_back(cnt_cols);
      const double scaled_width = std::max(
          (double)(workarea.width - (cnt_cols + 1) * spacing) / cnt_cols, 1.0);
      ...
      for (size_t j = 0; j < cnt_cols; j++) {
        ...
        for (auto &child : view->enumerate_views(true)) {
          ...
          if (child_data.visibility ==
            ...
            wf::scene::set_node_enabled(child->get_transformed_node(), true);
          }

          child_data.visibility = view_scale_data::view_visibility_t::VISIBLE;
          ...
          if (!allow_scale_zoom && (child != view) && (max_scale_child > 0.0)) {
          ...
          setup_view_transform(child_data, scale, scale, dx, dy, target_alpha);
        }
      }
    }
  ...
  wf::config::option_base_t::updated_callback_t
      allow_scale_zoom_option_changed = [=]() {
        if (!output->is_plugin_active(grab_interface.name)) {
          return;
        }
  ...
  /* View minimized */
  wf::signal::connection_t<wf::view_minimized_signal> view_minimized =
      [=](wf::view_minimized_signal *ev) {
        // Handle view restoration, view minimization is handled by disappeared
        // already.
  ...
  void refocus() {
    ...
    for (auto v : views) {
      if (v->is_mapped() && v->get_keyboard_focus_surface()) {
        next_focus = v;
        break;
      }
    }
  ...
  wf::effect_hook_t pre_hook = [=]() { transform_views(); };
  ...
  wf::signal::connection_t<wf::move_drag::drag_focus_output_signal>
      on_drag_output_focus = [=](wf::move_drag::drag_focus_output_signal *ev) {
  ...
  wf::signal::connection_t<wf::move_drag::snap_off_signal> on_drag_snap_off =
      [=](auto) { last_selected_view = nullptr; };
  ...
  bool activate() {
    ...
    if (active_view) {
      ...
      if (std::find(views.begin(), views.end(), current_focus_view) ==
          views.end()) {
        current_focus_view = nullptr;
      }
    } else {
  ...
  void deactivate() {
    ...
    for (auto &e : scale_data) {
      if (e.first->minimized && (e.first != current_focus_view)) {
        e.second.visibility = view_scale_data::view_visibility_t::HIDING;
...
public:
  ...
  wf::signal::connection_t<wf::view_set_output_signal> on_view_set_output =
      [=](wf::view_set_output_signal *ev) {
        if (auto toplevel = wf::toplevel_cast(ev->view)) {
          ...
          if (new_output && output_instance.count(new_output) &&
              output_instance[new_output]->active) {
            this->output_instance[ev->view->get_output()]->handle_new_view(
                toplevel, false);
          }
        }
      };
  ...
  wf::ipc_activator_t::handler_t toggle_all_cb = [=](wf::output_t *output,
                                                     wayfire_view) {
