...
public:
  ...
  headless_input_backend_t &
  operator=(const headless_input_backend_t &) = delete;
...
public:
  void init() override {
    ...
    method_repository->register_method("stipc/create_wayland_output",
                                       create_wayland_output);
    method_repository->register_method("stipc/destroy_wayland_output",
    ...
    method_repository->register_method("stipc/get_xwayland_pid",
                                       get_xwayland_pid);
    method_repository->register_method("stipc/get_xwayland_display",
  ...
  ipc::method_callback layout_views = [](wf::json_t data) -> wf::json_t {
    ...
    for (size_t i = 0; i < data["views"].size(); i++) {
      ...
      if (output.has_value()) {
        ...
        if (!wo) {
          return wf::ipc::json_error("Unknown output " +
  ...
  ipc::method_callback create_wayland_output = [](wf::json_t) {
    ...
    if (!wayland_backend) {
      return wf::ipc::json_error(
          "Wayfire is not running in nested wayland mode!");
    }
  ...
  public:
    ...
    std::string stringify() const override { return "force-timeout"; }
  };
