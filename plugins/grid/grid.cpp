...
nonstd::observer_ptr<wf::grid::grid_animation_t>
ensure_grid_view(wayfire_toplevel_view view) {
...
class wayfire_grid : public wf::plugin_interface_t,
  ...
  std::vector<std::string> slots = {"unused", "bl", "b",  "br", "l",
                                    "c",      "r",  "tl", "t",  "tr"};
  ...
  wf::option_wrapper_t<int> quarter_snap_threshold{
      "move/quarter_snap_threshold"};
  struct {
  ...
  wf::ipc_activator_t::handler_t handle_restore = [=](wf::output_t *wo,
                                                      wayfire_view view) {
...
public:
  ...
  wf::signal::connection_t<wf::grid::grid_handle_move_signal>
      grid_handle_move_signal_cb = [=](wf::grid::grid_handle_move_signal *ev) {
        ev->carried_out = true;
        wf::grid::slot_t new_slot_id =
            ev->operation == wf::grid::MOVE_OP_CLEAR_PREVIEW
                ? wf::grid::slot_t::SLOT_NONE
                : wf::grid::calc_slot(ev->output, ev->input, snap_threshold,
                                      quarter_snap_threshold);

        if ((ev->operation == wf::grid::MOVE_OP_DROP) && new_slot_id) {
          ev->view->toplevel()->pending().tiled_edges =
              wf::grid::get_tiled_edges_for_slot(new_slot_id);
          auto desired_size = wf::grid::get_slot_dimensions(
              ev->view->get_output(), new_slot_id);
          ev->view->get_data_safe<wf_grid_slot_data>()->slot = new_slot_id;
          ensure_grid_view(ev->view)->adjust_target_geometry(
              adjust_for_workspace(
                  ev->view->get_wset(), desired_size,
        ...
        if (new_slot_id) {
          wf::geometry_t slot_geometry =
              wf::grid::get_slot_dimensions(ev->output, new_slot_id);
  ...
  void fini() override { fini_output_tracking(); }
  ...
  bool can_adjust_view(wayfire_toplevel_view view) {
    ...
    const bool is_floating =
        (view->get_allowed_actions() & req_actions) == req_actions;
    return is_floating && (view->get_output() != nullptr) &&
           view->toplevel()->pending().mapped;
  }

  void handle_slot(wayfire_toplevel_view view, int slot,
                   wf::point_t delta = {0, 0}) {
    ...
    auto slot_geometry =
        wf::grid::get_slot_dimensions(view->get_output(), slot) + delta;
      ...
      [=](wf::workarea_changed_signal *ev) {
        for (auto &view : ev->output->wset()->get_views(wf::WSET_MAPPED_ONLY)) {
          ...
          if (view->pending_tiled_edges() &&
              (wm.width == ev->old_workarea.width) &&
          ...
          /* Workarea changed, and we have a view which is tiled into some
           * slot. We need to make sure it remains in its slot. So we
           * calculate the viewport of the view, and tile it there */
  ...
  wf::signal::connection_t<wf::view_tile_request_signal> on_maximize_signal =
      [=](wf::view_tile_request_signal *data) {
        if (data->carried_out || (data->desired_size.width <= 0) ||
            !data->view->get_output() || !data->view->get_wset() ||
        ...
        if (slot > 0) {
          data->desired_size =
              wf::grid::get_slot_dimensions(data->view->get_output(), slot);
        }

        data->view->get_data_safe<wf_grid_slot_data>()->slot = slot;
        ensure_grid_view(data->view)
            ->adjust_target_geometry(
                adjust_for_workspace(data->view->get_wset(), data->desired_size,
                                     data->workspace),
                wf::grid::get_tiled_edges_for_slot(slot));
      };

  wf::signal::connection_t<wf::view_fullscreen_request_signal>
      on_fullscreen_signal = [=](wf::view_fullscreen_request_signal *data) {
        ...
        ensure_grid_view(data->view)
            ->adjust_target_geometry(
                adjust_for_workspace(data->view->get_wset(), geom,
                                     data->workspace),
                edges);
      };
