...
public:
  ...
  std::string stringify() const override { return "crossfade"; }
...
public:
  crossfade_render_instance_t(crossfade_node_t *self,
                              scene::damage_callback push_damage) {
    this->self =
        std::dynamic_pointer_cast<crossfade_node_t>(self->shared_from_this());
  ...
  void
  schedule_instructions(std::vector<scene::render_instruction_t> &instructions,
  ...
  void render(const wf::scene::render_instruction_t &data) override {
    ...
    data.pass->add_texture(tex, data.target, self->displayed_geometry,
                           data.damage, 1.0 - ra);
  }
};
...
inline void crossfade_node_t::gen_render_instances(
  ...
  scene::view_2d_transformer_t::gen_render_instances(instances, push_damage,
                                                     shown_on);
}

/**
 * A class used for crossfade animation of a change in a view's geometry.
 */
...
public:
   ...
   * @param duration Indicates the duration of the animation (only for
   * crossfade)
   */
  ...
  void adjust_target_geometry(wf::geometry_t geometry, int32_t target_edges,
    ...
    ensure_view_transformer<crossfade_node_t>(view, wf::TRANSFORMER_2D, view);
...
protected:
  ...
  void destroy() { view->erase_data<grid_animation_t>(); }

  wf::geometry_t original;
  wayfire_toplevel_view view;
  wf::output_t *output;
  wf::signal::connection_t<view_disappeared_signal> on_disappear =
      [=](view_disappeared_signal *ev) {
        if (ev->view == view) {
          destroy();
        }
      };
