...
namespace wf {
namespace move_drag {
static wf::geometry_t find_geometry_around(wf::dimensions_t size,
                                           wf::point_t grab,
 ...
 *
 * It is primarily used to scale the view is a plugin needs it, and also to keep
 * it centered around the `grab_position`.
 */
...
public:
  ...
  std::string stringify() const override { return "move-drag"; }
  ...
  public:
    ...
    void render(const wf::scene::render_instruction_t &data) override {
      ...
      data.pass->add_texture(tex, data.target, bbox, data.damage,
                             self->alpha_factor);
    }
  };
  ...
  void gen_render_instances(std::vector<scene::render_instance_uptr> &instances,
    ...
    instances.push_back(
        std::make_unique<render_instance_t>(this, push_damage, shown_on));
  }
};
...
// The assumption is that all nodes have a view transformer which transforms
// them to global (not output-local) coordinates and thus we just need to
...
public:
  ...
  void gen_render_instances(std::vector<scene::render_instance_uptr> &instances,
    ...
    instances.push_back(std::make_unique<dragged_view_render_instance_t>(
        std::dynamic_pointer_cast<dragged_view_node_t>(shared_from_this()),
        push_damage, output));
  }
  ...
  class dragged_view_render_instance_t : public wf::scene::render_instance_t {
    ...
    wf::signal::connection_t<scene::node_damage_signal> on_node_damage =
        [=](scene::node_damage_signal *data) { push_damage(data->region); };
  ...
  public:
    dragged_view_render_instance_t(std::shared_ptr<dragged_view_node_t> self,
      ...
      this->children_manager =
          std::make_unique<wf::scene::render_instance_manager_t>(
              all_rendered, push_damage_child, shown_on);

      const int BIG_NUMBER = 1e5;
      wf::region_t big_region = wf::geometry_t{-BIG_NUMBER, -BIG_NUMBER,
                                               2 * BIG_NUMBER, 2 * BIG_NUMBER};
...
core_drag_t::core_drag_t() {
  ...
  priv->on_view_unmap = [=](auto *ev) { handle_input_released(); };
...
void core_drag_t::start_drag(wayfire_toplevel_view grab_view,
                             wf::pointf_t relative,
  ...
  for (auto &v : target_views) {
    ...
    v->get_transformed_node()->add_transformer(tr,
                                               wf::TRANSFORMER_HIGHLEVEL - 1);
...
void core_drag_t::start_drag(wayfire_toplevel_view view,
                             const drag_options_t &options) {
  wf::dassert(tentative_grab_position.has_value(),
              "First, the drag operation should be set as pending!");
...
void core_drag_t::handle_input_released() {
  ...
  for (auto &v : priv->all_views) {
    data.all_views.push_back({v.view, v.transformer->relative_grab});
  }
...
bool core_drag_t::is_view_held_in_place() { return priv->view_held_in_place; }
...
void adjust_view_on_output(drag_done_signal *ev) {
  ...
  for (auto &v : ev->all_views) {
    ...
    bbox = wf::move_drag::find_geometry_around(wf::dimensions(bbox), grab,
                                               v.relative_grab);

    wf::point_t target = wf::origin(bbox) + wm_offset;
    v.view->move(target.x, target.y);
    if (v.view->pending_fullscreen()) {
      wf::get_core().default_wm->fullscreen_request(v.view, ev->focused_output,
                                                    true, target_ws);
    } else if (v.view->pending_tiled_edges()) {
      wf::get_core().default_wm->tile_request(
          v.view, v.view->pending_tiled_edges(), target_ws);
    }
