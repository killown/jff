...
class wayfire_move : public wf::per_output_plugin_instance_t,
  ...
  wf::option_wrapper_t<int> workspace_switch_after{
      "move/workspace_switch_after"};
  ...
  wf::signal::connection_t<wf::move_drag::drag_focus_output_signal>
      on_drag_output_focus = [=](wf::move_drag::drag_focus_output_signal *ev) {
        if ((ev->focus_output == output) &&
            can_handle_drag(drag_helper->view)) {
...
public:
  void init() override {
    ...
    input_grab =
        std::make_unique<wf::input_grab_t>("move", output, nullptr, this);
  ...
  uint32_t get_act_flags(wayfire_toplevel_view view) {
    auto view_layer =
        wf::get_view_layer(view).value_or(wf::scene::layer::WORKSPACE);
  ...
  bool initiate(wayfire_toplevel_view view, wf::point_t grab_position) {
    auto target_output = wf::get_core().output_layout->get_output_at(
        grab_position.x, grab_position.y);
  ...
  void update_workspace_switch_timeout(wf::point_t input,
                                       bool force_disable_timer) {
    wf::grid::slot_t slot_id =
        force_disable_timer ? wf::grid::SLOT_NONE
                            : wf::grid::calc_slot(output, input, snap_threshold,
  ...
  bool is_snap_enabled() {
    if (!enable_snap || !drag_helper->view ||
        drag_helper->is_view_held_in_place()) {
      return false;
    }
