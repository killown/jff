...
class wayfire_expo : public wf::per_output_plugin_instance_t,
...
private:
  ...
  wf::option_wrapper_t<wf::animation_description_t> zoom_duration{
      "expo/duration"};
...
public:
  ...
  void init() override {
    input_grab = std::make_unique<wf::input_grab_t>("expo", output, this, this);
  ...
  void handle_keyboard_key(wf::seat_t *,
                           wlr_keyboard_key_event event) override {
    ...
    if (zooming && !this->state.zoom_in) {
      if (state == WLR_BUTTON_PRESSED) {
        ...
        if (update_target_workspace(x, y)) {
          ...
          // Calculate animation so that it appears as if we smoothly changed
          // destination
          wf::geometry_t cur_viewport = zoom_animation;
          wf::geometry_t target_viewport =
              wall->get_workspace_rectangle(target_ws);
          float cur_a = std::clamp(zoom_animation.progress(), 0.01, 0.99);
          wf::geometry_t start_viewport = wf::interpolate(
              cur_viewport, target_viewport, 1.0 - 1.0 / (1.0 - cur_a));
  ...
  void start_moving(wayfire_toplevel_view view, wf::point_t local_grab) {
    ...
    drag_helper->start_drag(
        view, wf::move_drag::find_relative_grab(bbox, ws_coords), opts);
  ...
  void handle_input_move(wf::point_t to) {
    ...
    if (!drag_helper->should_start_pending_drag(to) ||
        zoom_animation.running()) {
      /* Ignore small movements */
      return;
    }

    auto local_grab = *drag_helper->tentative_grab_position -
