...
class wayfire_resize : public wf::per_output_plugin_instance_t,
...
public:
  void init() override {
    input_grab =
        std::make_unique<wf::input_grab_t>("resize", output, nullptr, this);

    activate_binding = [=](auto) { return activate(false); };

    activate_binding_preserve_aspect = [=](auto) { return activate(true); };

    output->add_button(button, &activate_binding);
    output->add_button(button_preserve_aspect,
                       &activate_binding_preserve_aspect);
    grab_interface.cancel = [=]() { input_pressed(WLR_BUTTON_RELEASED); };
  ...
  void handle_pointer_button(const wlr_pointer_button_event &event) override {
    if ((event.state == WL_POINTER_BUTTON_STATE_RELEASED) &&
        was_client_request && (event.button == BTN_LEFT)) {
      return input_pressed(event.state);
    }

    if ((event.button != wf::buttonbinding_t(button).get_button()) &&
        (event.button !=
         wf::buttonbinding_t(button_preserve_aspect).get_button())) {
      return;
    }
  ...
  wf::point_t get_global_input_coords() {
    wf::pointf_t input = wf::get_core().get_cursor_position();
  ...
  bool initiate(wayfire_toplevel_view view, uint32_t forced_edges = 0) {
    ...
    this->edges =
        forced_edges
            ?: calculate_edges(view->get_bounding_box(), get_input_coords().x,
                               get_input_coords().y);
  ...
  wf::dimensions_t calculate_min_size() {
    ...
    min_size = wf::expand_dimensions_by_margins(
        min_size, view->toplevel()->pending().margins);

    min_size.width = std::max(min_size.width, static_cast<int>(user_min_width));
    min_size.height =
        std::max(min_size.height, static_cast<int>(user_min_height));
  ...
  wf::dimensions_t calculate_max_size(wf::dimensions_t min) {
    ...
    max_size.width = static_cast<decltype(max_size.width)>(
        std::clamp(width, static_cast<int64_t>(min.width),
                   static_cast<int64_t>(
                       std::numeric_limits<decltype(max_size.width)>::max())));
    max_size.height = static_cast<decltype(max_size.height)>(
        std::clamp(height, static_cast<int64_t>(min.height),
                   static_cast<int64_t>(
                       std::numeric_limits<decltype(max_size.height)>::max())));
  ...
  void input_motion() {
    ...
    if (preserve_aspect) {
      ...
      if ((new_ratio < ratio) ^
          does_shrink(desired.width - grabbed_geometry.width,
                      desired.height - grabbed_geometry.height)) {
        desired.width = std::clamp(int(desired.height * ratio), min_size.width,
                                   max_size.width);
        desired.height = std::clamp(int(desired.width / ratio), min_size.height,
                                    max_size.height);
        desired.width = std::clamp(int(desired.height * ratio), min_size.width,
                                   max_size.width);
      } else {
      ...
      desired.height =
          std::clamp(desired.height, min_size.height, max_size.height);
    }
    ...
    if (wf::dimensions(view->toplevel()->pending().geometry) !=
        wf::dimensions(desired)) {
