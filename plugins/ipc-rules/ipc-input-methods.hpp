...
public:
  void init_input_methods(ipc::method_repository_t *method_repository) {
    method_repository->register_method("input/list-devices",
                                       list_input_devices);
    method_repository->register_method("input/configure-device",
  ...
  static std::string
  wlr_input_device_type_to_string(wlr_input_device_type type) {
    switch (type) {
  ...
  wf::ipc::method_callback list_input_devices = [&](const wf::json_t &) {
    ...
    for (auto &device : wf::get_core().get_input_devices()) {
      ...
      if (wlr_input_device_is_libinput(device->get_wlr_handle())) {
        if (auto libinput_handle =
                wlr_libinput_get_device_handle(device->get_wlr_handle())) {
      ...
      d["type"] =
          wlr_input_device_type_to_string(device->get_wlr_handle()->type);
  ...
  wf::ipc::method_callback configure_input_device =
      [&](const wf::json_t &data) {
