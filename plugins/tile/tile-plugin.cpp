...
static bool can_tile_view(wayfire_toplevel_view view) {
  ...
  if ((view->toplevel()->get_min_size() == view->toplevel()->get_max_size()) &&
      (view->toplevel()->get_min_size().width > 0) &&
      (view->toplevel()->get_min_size().height > 0)) {
    return false;
  }
...
namespace wf {
class tile_output_plugin_t : public wf::pointer_interaction_t,
...
private:
  ...
  wf::option_wrapper_t<wf::buttonbinding_t> button_resize{
  ...
  wf::option_wrapper_t<wf::keybinding_t> key_focus_below{
      "simple-tile/key_focus_below"};

  wf::option_wrapper_t<wf::activatorbinding_t> key_toggle_maximized{
      "simple-tile/key_toggle_maximized"};
  wf::output_t *output;
...
public:
  ...
  template <class Controller> void start_controller() {
    ...
    controller =
        std::make_unique<Controller>(output->wset().get(), tiled_focus);
  }
  ...
  void attach_view(wayfire_toplevel_view view,
                   std::optional<wf::point_t> vp = {}) {
    if (!view->get_wset()) {
      return;
    }
  ...
  void detach_view(wayfire_toplevel_view view, bool reinsert = true) {
    ...
    // Note that stopping the controller might untile the view, or change its
    // tiled node.
    if (auto node = tile::view_node_t::get_node(view)) {
      tile_workspace_set_data_t::get(view->get_wset())
          .detach_views({node}, reinsert);
    }
  }

  wf::signal::connection_t<view_mapped_signal> on_view_mapped =
      [=](view_mapped_signal *ev) {
  ...
  wf::signal::connection_t<view_fullscreen_request_signal>
      on_fullscreen_request = [=](view_fullscreen_request_signal *ev) {
        if (ev->carried_out || !tile::view_node_t::get_node(ev->view)) {
          return;
        }

        ev->carried_out = true;
        tile_workspace_set_data_t::get(ev->view->get_wset())
            .set_view_fullscreen(ev->view, ev->state);
      };

  void change_view_workspace(wayfire_toplevel_view view,
                             std::optional<wf::point_t> vp = {}) {
  ...
  wf::signal::connection_t<view_change_workspace_signal>
      on_view_change_workspace = [=](view_change_workspace_signal *ev) {
        if (ev->old_workspace_valid) {
          change_view_workspace(ev->view, ev->to);
        }
      };

  wf::signal::connection_t<view_minimized_signal> on_view_minimized =
      [=](view_minimized_signal *ev) {
   ...
   */
  bool
  conditioned_view_execute(bool need_tiled,
  ...
  bool focus_adjacent(tile::split_insertion_t direction) {
    return conditioned_view_execute(true, [=](wayfire_toplevel_view view) {
      ...
      if (was_fullscreen && keep_fullscreen_on_adjacent) {
        wf::get_core().default_wm->fullscreen_request(adjacent->view, output,
                                                      true);
      }
...
public:
  tile_output_plugin_t(wf::output_t *wo) {
    this->output = wo;
    input_grab = std::make_unique<wf::input_grab_t>("simple-tile", output,
                                                    nullptr, this);
...
public:
  void init() override {
    ...
    ipc_repo->register_method("simple-tile/set-show-maximized",
                              ipc_set_show_maximized);
  ...
  wf::signal::connection_t<view_unmapped_signal> on_view_unmapped =
      [=](wf::view_unmapped_signal *ev) {
        ...
        if (wf::tile::view_node_t::get_node(ev->view)) {
          ...
          if (wo && (toplevel->get_wset() == wo->wset())) {
            ...
            tile_workspace_set_data_t::get(toplevel->get_wset())
                .detach_views({wf::tile::view_node_t::get_node(ev->view)});
          }
        }
      };

  wf::signal::connection_t<view_pre_moved_to_wset_signal>
      on_view_pre_moved_to_wset = [=](view_pre_moved_to_wset_signal *ev) {
      ...
      [=](keyboard_focus_changed_signal *ev) {
        if (auto toplevel = toplevel_cast(wf::node_to_view(ev->new_focus))) {
          if (toplevel->get_wset()) {
            ...
            tile_workspace_set_data_t::get(wset).consider_exit_fullscreen(
                toplevel);
          }
        }
      };
  ...
  ipc::method_callback ipc_set_show_maximized =
      [=](wf::json_t params) -> wf::json_t {
    return tile::handle_ipc_set_show_maximized(params);
  };
};

std::unique_ptr<tile::tree_node_t> &tile::get_root(wf::workspace_set_t *set,
                                                   wf::point_t workspace) {
  return tile_workspace_set_data_t::get(set->shared_from_this())
      .roots[workspace.x][workspace.y];
}
} // namespace wf
